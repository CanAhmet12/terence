...
export default function AdminIcerikYuklePage() {
  ...
  const upload = async () => {
    ...
    try {
      ...
      setMsg({ type: "ok", text: "İçerik yüklendi. Kapak sunucuda otomatik oluşturuldu (video için özel kapak yüklemediyseniz)." });
      setFile(null);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 xl:p-12 max-w-2xl min-w-0 overflow-x-hidden">
      <Link href="/admin/icerik-merkezi" className="inline-flex items-center gap-2 text-slate-600 hover:text-teal-600 mb-6 font-medium text-sm">
        <ArrowLeft className="w-4 h-4" />
        İçerik merkezine dön
      </Link>
      <h1 className="text-2xl font-bold text-slate-900 mb-2">Müfredat medyası yükle</h1>
      <p className="text-sm text-slate-600 mb-6">
        Yalnızca dosya yükleme; video ve PDF doğrudan depoya alınır ve müfredat konusuna bağlanır.
      </p>

      {msg && (
        <div
          className={`mb-4 p-4 rounded-xl flex items-start gap-2 text-sm font-medium ${
            msg.type === "ok" ? "bg-teal-50 text-teal-800 border border-teal-100" : "bg-red-50 text-red-700 border border-red-100"
          }`}
        >
      ...
      <div className="space-y-4 bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
                ...
                <li key={t.id}>
                  ...
                  >
                    ...
                    <span className="text-xs text-slate-500">
                      {t.subject_name} · {t.unit_title} · {t.meb_code || "—"}
                    </span>
                  </button>
                </li>
              ))}
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border text-sm font-semibold ${
              contentType === "video" ? "border-teal-500 bg-teal-50 text-teal-800" : "border-slate-200"
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border text-sm font-semibold ${
              contentType === "pdf" ? "border-amber-500 bg-amber-50 text-amber-900" : "border-slate-200"
            }`}
          >
        ...
        <div>
          <label className="text-xs font-bold text-slate-700">Dosya (zorunlu)</label>
          <input
            type="file"
        ...
        {contentType === "video" && (
          <div className="rounded-xl border border-slate-100 bg-slate-50/80 p-4 space-y-3">
            <p className="text-xs font-bold text-slate-700 flex items-center gap-2">
              <ImageIcon className="w-3.5 h-3.5" /> Özel kapak (opsiyonel)
            </p>
            <p className="text-[11px] text-slate-500">Boş bırakırsanız sunucu otomatik kapak üretir.</p>
            <input
              type="file"
