...
import { GraduationCap, Mail, Lock, Eye, EyeOff, Smartphone, Phone, ArrowRight } from "lucide-react";
...
export default function LoginPage() {
  ...
  if (user) {
    ...
    else if (user.role === "student") router.replace("/ogrenci");
    else router.replace("/");
    return null;
  }
    ...
    <div className="min-h-screen flex">
      <div className="flex-1 flex items-center justify-center p-8 bg-white">
        <div className="w-full max-w-md">
          ...
          <div className="flex gap-1 p-1.5 bg-slate-100 rounded-xl mb-6">
            <button
              type="button"
              onClick={() => setLoginMode("email")}
              className={`flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all ${
                loginMode === "email" ? "bg-white text-teal-700 shadow-md" : "text-slate-600 hover:text-slate-900"
              }`}
            >
              E-posta
            </button>
            <button
              ...
              className={`flex-1 py-2.5 rounded-lg text-sm font-semibold transition-all ${
                loginMode === "phone" ? "bg-white text-teal-700 shadow-md" : "text-slate-600 hover:text-slate-900"
              }`}
            >
              Telefon
            </button>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {loginMode === "email" ? (
              ...
              <div className="space-y-5">
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-2">SMS Kodu</label>
                  <input
                    type="text"
                    value={smsCode}
                ...
                <p className="text-xs text-slate-500">Telefon ile giriş için SMS doğrulama gerekir. Backend entegrasyonu sonrası aktif olacak.</p>
                <button
                  type="button"
                  onClick={() => setLoginMode("email")}
                  className="w-full py-3.5 border-2 border-slate-200 rounded-xl text-slate-700 font-medium hover:bg-slate-50 transition-colors"
                >
            ...
            <div className="flex flex-wrap gap-2">
              {["student", "teacher", "admin", "parent"].map((role) => (
                <button
                  key={role}
                  type="button"
                  onClick={() => handleDemo(role as "student" | "teacher" | "admin" | "parent")}
          ...
          <div className="mt-6 pt-6 border-t border-slate-200">
            <p className="text-sm text-slate-500 text-center mb-3">veya</p>
            <div className="flex flex-col sm:flex-row gap-2">
              <button
                type="button"
                className="flex-1 flex items-center justify-center gap-2 py-3 border-2 border-slate-200 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors"
              >
